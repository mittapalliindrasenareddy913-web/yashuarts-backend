...
router.put('/fcm-token', protect, async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      await user.save();
      console.log(`[FCM] FCM token saved for user ${user._id}`);
...
router.post('/test-notification', protect, async (req, res) => {
  ...
  try {
    ...
    // We dynamically import to avoid circular dependencies if needed,
    // but we can also just import at the top of the file.
    // Wait, let's just use the existing push notification utility if possible.
    // I will add the import at the top later.
    const { sendPushNotification } = await import('../utils/notifications.js');
    
    await sendPushNotification(req.app, targetUserId, 'Test Notification 🔔', 'This is a test push notification from YashuArts!');
